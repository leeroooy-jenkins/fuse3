...
Запись на диск делает сам `SpyFs`. Сверх этого каждая запись отдаётся списку экшенов,
который собирается в `Main` — порядок в списке и есть порядок выполнения: `PartLogAction`
(лог частей), `S3Action` (multipart-загрузка). Ненужный экшен убирается оттуда же. `S3Action` требует переменных окружения `S3_ENDPOINT`, `S3_BUCKET`, `S3_ACCESS_KEY`,
`S3_SECRET_KEY`. Путь `/<uuid>/screen` кладётся в бакет ключом `screen/<uuid>`.
